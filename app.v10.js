...
function renderDash(c) {
  ...
  c.innerHTML = `
    ...
    <div style="margin-bottom:18px">
      <div style="font-size:9px;font-weight:700;color:var(--v6-t3);font-family:var(--font-mono);letter-spacing:.14em;text-transform:uppercase;margin-bottom:4px">${fr ? 'TABLEAU DE BORD' : 'DASHBOARD'}</div>
      <div style="font-size:20px;font-weight:800;color:var(--v6-t1);letter-spacing:-.03em">Predictive Intelligence <span style="color:var(--v6-teal)">Hybrid</span></div>
    </div>

    <!-- KPI GRID -->
    <div class="kpi-grid">
      <div class="kpi-card kc-teal">
        <div class="kpi-lbl">${fr ? 'SCANS LIVE' : 'LIVE SCANS'}</div>
        <div class="kpi-val" style="color:var(--v6-teal)">${all.length}</div>
        <div class="kpi-sub">${fr ? 'events actifs' : 'active events'}</div>
        <div class="kpi-badge kb-teal">● live</div>
      </div>
      <div class="kpi-card kc-gold">
        <div class="kpi-lbl">${fr ? 'OPPORTUNITÉS' : 'OPPORTUNITIES'}</div>
        <div class="kpi-val" style="color:var(--v6-gold2)">${hot}</div>
        <div class="kpi-sub">marge &gt; 100%</div>
        <div class="kpi-badge kb-gold">premium</div>
      </div>
      <div class="kpi-card kc-purple">
        <div class="kpi-lbl">AVG ROI · IA</div>
        <div class="kpi-val" style="color:var(--v6-purple)">+${avg}%</div>
        <div class="kpi-sub">${fr ? 'marge nette est.' : 'est. net margin'}</div>
        <div class="kpi-badge kb-purple">↑ ${fr ? 'prédictif' : 'predictive'}</div>
      </div>
      <div class="kpi-card kc-red">
        <div class="kpi-lbl">ALERTS</div>
        <div class="kpi-val" style="color:var(--v6-red)">${alerts.length || drops}</div>
        <div class="kpi-sub">presales + ${fr ? 'chutes' : 'drops'}</div>
        <div class="kpi-badge kb-red">${upcoming} presales J-7</div>
      </div>
    </div>
    ...
    <div style="margin-top:14px">
      <div class="ai-bar">
        ...
        <button class="ai-bar-btn" onclick="runDashAI()">${fr ? 'Analyser →' : 'Analyze →'}</button>
      </div>
      <div class="ai-suggestions">
        ${suggestions.map(s => `<div class="ai-sug" onclick="document.getElementById('ai-dash-input').value='${s}'">${s}</div>`).join('')}
      </div>
      <div id="ai-dash-resp" style="display:none;margin:0 0 12px;background:var(--v6-bg3);border:1px solid rgba(167,139,250,.2);border-radius:10px;padding:12px 14px;font-size:11.5px;color:var(--v6-t2);font-family:var(--font-mono);line-height:1.6"></div>
    </div>
    ...
    <div class="map-card">
      ...
      <div class="map-body" style="height:220px;overflow:hidden;position:relative">
        ...
        <div class="ai-tooltip-card" id="ai-map-tooltip" style="display:none">
    ...
    <!-- TRENDING EVENTS -->
    <div class="trending-header" style="display:flex;align-items:center;justify-content:space-between;margin:16px 0 10px">
      <div>
    ...
    <div class="ev-cards-grid" style="display:grid;grid-template-columns:repeat(auto-fill,minmax(220px,1fr));gap:10px">
      ${trending.map(ev => {
        const s = sig(ev);
        const sc = sigColor(s);
