...
/* ══════════════════════════════════════════════
   DATA
══════════════════════════════════════════════ */
// FALLBACK_EVENTS moved to config.js
...
// MARKETS moved to config.js
const MARKETS = CONFIG.MARKETS.map(m => ({...m}));
const _MARKETS_OLD = [
  {id:'FR',label:'France 🇫🇷',on:true},{id:'UK',label:'UK 🇬🇧',on:true},
  {id:'US',label:'USA 🇺🇸',on:true},{id:'ES',label:'Espagne 🇪🇸',on:true},
  {id:'F1',label:'F1 🏎️',on:true},{id:'MMA',label:'MMA/UFC 🥊',on:true},
  {id:'AS',label:'Asia 🌏',on:true},
];

// PLATFORMS moved to config.js
...
// FX moved to config.js
const FX = CONFIG.FX;
const _FX_OLD = {EUR_USD:1.085,EUR_GBP:0.856,USD_EUR:0.922,USD_GBP:0.789,GBP_EUR:1.168,GBP_USD:1.267};
...
const S = {
  ...
  sheetUrl: localStorage.getItem('tr-sheet-url') || SHEET_CSV_URL || '',
  tgToken: localStorage.getItem('tr-tg-token') || '',
  tgChatId: localStorage.getItem('tr-tg-chatid') || '',
  apiUrl: localStorage.getItem('tr-api-url') || SCRAPER_API_URL || '',
